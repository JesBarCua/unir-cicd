...
test-e2e:
	...
	docker run -d --network calc-test-e2e --name calc-web -p 82:80 calc-web
	docker create --network calc-test-e2e --name e2e-tests cypress/included:4.9.0 || true
...
run-web:
	docker run --rm --network apiserver --volume `pwd`/web:/usr/share/nginx/html  --volume `pwd`/web/constants.local.js:/usr/share/nginx/html/constants.js --name calc-web -p 82:80 nginx
